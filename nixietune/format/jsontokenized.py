...
class JSONTokenizedDataset:
    ...
    def load(path: str, tok: PreTrainedTokenizerBase, split: str, max_len: int, num_workers: int = 4) -> Dataset:
        if os.path.exists(path):
            ...
            return JSONTokenizedDataset.from_dataset(dataset, tokenizer=tok, max_len=max_len, num_workers=num_workers)

    @staticmethod
    def from_dataset(ds: Dataset, tokenizer: PreTrainedTokenizerBase, max_len: int, num_workers: int = 4) -> Dataset:
        return JSONTokenizedDataset._tokenize(ds, tokenizer, max_len, num_workers)
    ...
    def from_file(
        ...
        return JSONTokenizedDataset._tokenize(source, tokenizer, max_len, num_workers)
    ...
    def _tokenize(data: Dataset, tokenizer: PreTrainedTokenizerBase, max_len: int, num_workers: int) -> Dataset:
        def process_batch(batch: Dict[str, List]) -> Dict[str, List]:
            ...
            positive = tokenizer(batch["doc"], padding=False, truncation=True, max_length=max_len)["input_ids"]
